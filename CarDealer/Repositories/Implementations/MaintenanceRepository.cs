...
public class MaintenanceRepository : Repository<Maintenance>, IMaintenanceRepository
{
    ...
    public async Task<Maintenance?> GetTrackedWithPaymentsAsync(int id) =>
        await _context.Maintenances
            .Include(m => m.Payments)
            .Include(m => m.MaintenanceCenter)
            .FirstOrDefaultAsync(m => m.Id == id);

    public async Task<List<Maintenance>> GetByCenterIdWithDetailsAsync(int centerId) =>
        await _context.Maintenances
            .AsNoTracking()
            .Include(m => m.MaintenanceCenter)
            .Include(m => m.Payments)
    ...
    public async Task<List<Maintenance>> GetForDebtReportAsync(
